...
| FR-8 | The governance core and the actuation harness import nothing; the gate adapter imports only the runtime and the core; the simulation, data-plane and demo adapters do not import each other. The one permitted external import in an adapter is the optional reference consumer library in `dataplane/measure.js`, named explicitly. | F7, 40 static import checks of the real import graph (the count grows as files are added — F7 auto-scans) |
| FR-9 | The simulation compares the policies on the identical task list per seed: always-run (P0), threshold deferral on a whole-window median (P1), threshold deferral on a trailing 7-day median with no lookahead (P1t), the governor at five budget levels (P2), plus the arms added after submission — the tiered rule (P2tiered), the argmin objective (P3, and the E2b sweep), and the real-trace granularity pair (P2real/P2equal6). | `simulation/run.js`; the rung semantics are asserted in `simulation/policies.test.js`; results in `results/simulation.md` |
...
| NFR-3 | **Readability.** Files stay small and single-purpose. Roughly 150 lines is the *target*, not a ceiling nothing enforces: twelve source files are above it today (the experiment files, the two registries, the renderers and the capture tools), each listed in ADR-001 with its reason. Nothing else may cross it without joining that list. The governor core is 104 lines. | Reviewed on every change; ADR-001 holds the exception list; `npm run arch` checks for cycles and `npm run arch:graph` draws the whole graph |
...
- An experiment that decides through the forecast adapter. The port itself is built (WP-3: contract, capture, conformance suite); nothing consumes it on a decided path yet.
...
| The forecast port | `docs/ports/FORECAST.md`, `simulation/forecast.js` | **Built (WP-3): contract + committed capture + conformance-tested adapter.** The simulations still read the peer signal straight out of the cached trace — nothing decides through the adapter yet. |
