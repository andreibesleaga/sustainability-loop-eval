...
export function renderSimulationMd(doc) {
  ...
  for (const id of Object.keys(doc.results)) {
    ...
    out += `| policy | total gCO2e | % vs P0 | completed | dropped | degraded | deferred | mean delay (min) | p95 delay | human decisions | days over budget | audit valid |\n|---|---|---|---|---|---|---|---|---|---|---|---|\n`;
    ...
    out += `\nTasks per seed: ${v(win.tasksPerSeed)}. P1 threshold (${p.days}-day median peer signal): ${win.peerMedianThresholdG} gCO2e/kWh.\n\n`;
  }
...
function summaryProse(doc) {
  ...
  const L = [
    ...
    `2. Carbon-aware deferral alone (P1) is weak on these traces: ${g("W1", "P1", "pctVsP0")}% winter and ${g("W2", "P1", "pctVsP0")}% summer, at zero cost in completed work. Half the workload is non-deferrable, and a median threshold moves work to the first *acceptable* slot, not the cleanest one.`,
    ...
    `8. Deferred work is genuinely delayed: mean ${g("W1", "P2_f0.8", "meanDelayMin")} min and p95 at the ${horizonMin}-min horizon cap. Every completed task still met its deadline, because deadlines are hard by construction.`,
