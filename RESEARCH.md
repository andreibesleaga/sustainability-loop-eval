...
- **The safety properties hold in shipped code.** Thirteen executable checks against the real gate, all green over 14,981 cases. The gate always picks the worst verdict, refuses on bad input instead of allowing, never runs anything above "degrade" without a human, never runs "terminate" at all, and its audit log catches tampering. (Nine of the thirteen, over 10,994 cases, are what the article cites; four were added afterwards. See **Versions** below.)
...
| `npm test` | 60 unit tests for the adapters' own arithmetic, then the thirteen architecture checks through the real `kaiban-distributed` gate, then `check:docs`. | 13/13 green over 14,981 cases; the runtime's own governance suite (4 files, 71 tests) passes at commit `17ad362` |
...
- **Designed, not built:** the wiring from the gate to a real approval board, the actuation edge to the charging tool servers, and the normative criteria above. (The forecast port left this list on 2026-09-02: WP-3 built it — [docs/ports/FORECAST.md](docs/ports/FORECAST.md), a committed live capture, and an offline reference adapter with its conformance suite. The metering port's contract followed — [docs/ports/METERING.md](docs/ports/METERING.md).)
